...
def aggregate_data(data_source, data_path=None, sql_query=None, meta_data):
  """Performs nested aggregation on provided data with potentially nested children.

  Args:
      data_source (str): "pandas" or "spark".
      data_path (str, optional): Path to the CSV or Excel file for Pandas. Defaults to None.
      sql_query (str, optional): SQL query for Spark. Defaults to None.
      meta_data (dict): Dictionary defining nested aggregation logic with potentially nested children.

  Returns:
      pandas.core.frame.DataFrame (if data_source is "pandas") or
      pyspark.sql.dataframe.DataFrame (if data_source is "spark").
  """
  ...
  def nested_aggregate(df, meta, prefix=""):
    if isinstance(meta, dict):
      ...
      # Join with child tables and perform nested aggregation recursively
  ...
  "children": [
    {
      ...
      "children": [  # Nested children under child_1
        {
          "id": "grandchild_1",
          "link": ["col7"],
          "aggregate": {"col8": "min"},
          "rename": {"col8": "min_col8"}
        }
      ]
    }
  ]
